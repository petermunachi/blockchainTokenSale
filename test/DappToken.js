...
contract('DappToken', function (accounts) {
  ...
  it('Transfer Token ownership', function () {
    return DappToken.deployed().then(function (instance) {
      tokenInstance = instance;
      // Test 'require' statement first by transferring something larger than the seender's balance
  ...
  it('approves tokens for delegated transfer', function () {
    return DappToken.deployed().then(function (instance) {
      tokenInstance = instance;
      //triggers transaction but doesn't modify data
  ...
  it('handles delegated token transfers', function () {
    return DappToken.deployed().then(function (instance) {
      ...
      assert.equal(balance.toNumber(), 90, 'deducts the amount to the sending account');
      return tokenInstance.balanceOf(toAccount);
    }).then(function (balance) {
      assert.equal(balance.toNumber(), 10, 'increase the amount of the receiving account');
